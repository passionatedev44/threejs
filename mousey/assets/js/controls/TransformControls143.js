				...
				if ( this.translationSnap ) {
					...
					if ( space === 'local' ) {
						...
						if ( axis.search( 'X' ) !== - 1 ) {
							...
							object.position.x = parseFloat(Math.round( object.position.x / this.translationSnap ) * this.translationSnap);

						}

						if ( axis.search( 'Y' ) !== - 1 ) {

							object.position.y = parseFloat(Math.round( object.position.y / this.translationSnap ) * this.translationSnap);

						}

						if ( axis.search( 'Z' ) !== - 1 ) {

							object.position.z = parseFloat(Math.round( object.position.z / this.translationSnap ) * this.translationSnap);
				...
				for ( const name in gizmoMap ) {
					...
					for ( let i = gizmoMap[ name ].length; i --; ) {
						...
						if ( position ) {
							...
							object.position.set( parseFloat(position[ 0 ]), parseFloat(position[ 1 ]), parseFloat(position[ 2 ]) );
